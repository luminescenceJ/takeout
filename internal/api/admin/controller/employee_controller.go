...
func (ec *EmployeeController) Login(ctx *gin.Context) {
	...
	if err != nil {
		code = e.ERROR
		global.Log.Debug("Invalid request payload:", err.Error()) // Log for invalid request payload
	...
	global.Log.Info("Employee login attempt:", "username", employeeLogin.UserName) // Log username for login attempt

	resp, err := ec.service.Login(ctx, employeeLogin)
	if err != nil {
		code = e.ERROR
		global.Log.Warn("EmployeeController login failed for username:", employeeLogin.UserName, "Error:", err.Error()) // Log failed login
	...
	global.Log.Info("Employee login successful:", "username", employeeLogin.UserName) // Log successful login
...
func (ec *EmployeeController) Logout(ctx *gin.Context) {
	...
	if err != nil {
		code = e.ERROR
		global.Log.Warn("EmployeeController logout failed:", err.Error()) // Log logout failure
		ctx.JSON(http.StatusBadRequest, common.Result{
			Code: code,
			Msg:  err.Error(),
		})
	...
	global.Log.Info("Employee successfully logged out.") // Log successful logout
	ctx.JSON(http.StatusOK, common.Result{
		Code: code,
	})
}
...
func (ec *EmployeeController) AddEmployee(ctx *gin.Context) {
	...
	if err != nil {
		global.Log.Debug("AddEmployee Bind Error:", err.Error()) // Log binding errors for employee creation
		ctx.JSON(http.StatusBadRequest, common.Result{
			Code: e.ERROR,
			Msg:  "Invalid request payload",
		})
	...
	global.Log.Info("Adding new employee:", "name", employee.Name, "username", employee.UserName) // Log employee creation attempt

	err = ec.service.CreateEmployee(ctx, employee)
	if err != nil {
		code = e.ERROR
		global.Log.Warn("AddEmployee failed for username:", employee.UserName, "Error:", err.Error()) // Log failed employee creation
		ctx.JSON(http.StatusOK, common.Result{
			Code: code,
			Msg:  err.Error(),
		})
	...
	global.Log.Info("Employee successfully added:", "username", employee.UserName) // Log successful employee creation
	ctx.JSON(http.StatusOK, common.Result{
		Code: code,
	})
}
...
func (ec *EmployeeController) PageQuery(ctx *gin.Context) {
	...
	if err != nil {
		code = e.ERROR
		global.Log.Error("PageQuery invalid params error:", err.Error()) // Log error for invalid query params
		ctx.JSON(http.StatusOK, common.Result{
			Code: code,
			Msg:  e.GetMsg(code),
		})
	...
	if err != nil {
		code = e.ERROR
		global.Log.Warn("PageQuery failed:", err.Error()) // Log error during page query
	...
	global.Log.Info("Successfully queried employee page:", "pageResult", pageResult) // Log successful page query
	ctx.JSON(http.StatusOK, common.Result{
		Code: code,
		Data: pageResult,
	})
...
func (ec *EmployeeController) OnOrOff(ctx *gin.Context) {
	...
	if err != nil {
		code = e.ERROR
		global.Log.Error("OnOrOff invalid params error:", err.Error()) // Log invalid parameter error
		ctx.JSON(http.StatusOK, common.Result{
			Code: code,
			Msg:  e.GetMsg(code),
		})
	...
	if err != nil {
		code = e.ERROR
		global.Log.Warn("OnOrOff status update failed for id:", id, "Error:", err.Error()) // Log failed status change
	...
	global.Log.Info("Updated employee status:", "id", id, "status", status)
	ctx.JSON(http.StatusOK, common.Result{
		Code: code,
	})
}
...
func (ec *EmployeeController) GetById(ctx *gin.Context) {
	...
	if err != nil {
		code = e.ERROR
		global.Log.Error("GetById invalid params error:", err.Error()) // Log invalid param error
		ctx.JSON(http.StatusBadRequest, common.Result{
			Code: code,
			Msg:  e.GetMsg(code),
		})
	...
	if err != nil {
		code = e.ERROR
		global.Log.Warn("GetById failed for id:", id, "Error:", err.Error()) // Log error fetching employee
		ctx.JSON(http.StatusInternalServerError, common.Result{
			Code: code,
			Msg:  err.Error(),
		})
	...
	// Log employee data retrieved successfully
	global.Log.Info("Successfully retrieved employee data:", "id", id, "employee", employee)
	ctx.JSON(http.StatusOK, common.Result{
		Code: code,
		Data: employee,
	})
...
func (ec *EmployeeController) UpdateEmployee(ctx *gin.Context) {
	...
	if err != nil {
		code = e.ERROR
		global.Log.Error("UpdateEmployee Bind Params Error:", err.Error()) // Log bind params error
		ctx.JSON(http.StatusBadRequest, common.Result{
			Code: code,
			Msg:  e.GetMsg(code),
		})
	...
	if err != nil {
		code = e.ERROR
		global.Log.Warn("UpdateEmployee failed:", err.Error()) // Log employee update failure
		ctx.JSON(http.StatusInternalServerError, common.Result{
			Code: code,
			Msg:  e.GetMsg(code),
		})
	...
	global.Log.Info("Employee information updated successfully.") // Log successful employee update
	ctx.JSON(http.StatusOK, common.Result{
		Code: code,
		Msg:  e.GetMsg(code),
	})
...
func (ec *EmployeeController) EditPassword(ctx *gin.Context) {
	...
	if err != nil {
		code = e.ERROR
		global.Log.Error("EditPassword Bind Params Error:", err.Error()) // Log bind params error
		ctx.JSON(http.StatusBadRequest, common.Result{
			Code: code,
			Msg:  e.GetMsg(code),
		})
	...
	if err != nil {
		code = e.ERROR
		global.Log.Error("Wrong Password Error:", err.Error()) // Log wrong password error
	...
	global.Log.Info("Employee password updated successfully.") // Log successful password change
	ctx.JSON(http.StatusOK, common.Result{
		Code: code,
	})
}
